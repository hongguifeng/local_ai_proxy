...
在 **历史日志** 中，每个 task 只显示整个 task 的已知金额合计，不受搜索结果或已加载页影响。点击费用可查看总额、已知/未计价/进行中数量、原因和冻结后的实际价格分组；task 汇总明细在表尾显示总计。每项费用明细均显示其占总费用的比例。选择请求后，可通过响应栏的 **¥** 按钮按需展开费用明细，查看其冻结模型、usage、实际单价和分项。所有单价和总价均为应用倍率后的实际价格。人民币费用最多显示四位小数。定价功能上线前的日志保留 `legacy_record`，不会按当前价格回算。
...
- 自动刷新。
- 输入关键词后按回车（或点击搜索按钮），按 method、path、status、target URL、task id、record id 搜索。搜索使用全文（token 级）匹配：空格分隔的关键词之间为 AND，每个关键词匹配完整 token 或其前缀；搜索结果中的每个分组都至少包含一条匹配记录。搜索进行中时搜索框下方会显示一条动画进度条，完成后自动隐藏。
- 大日志目录分页加载。
- 对相关 Agent 工作流进行任务分组。
...
历史日志页面支持为单条请求调用配置的总结模型，按连续消息区间生成阶段摘要，并缓存结果。已生成总结的请求会在任务的请求列表中以金色星标标识。
