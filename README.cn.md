...
- 自动刷新。
- 输入关键词后点击搜索按钮，按 method、path、status、target URL、task id、record id 搜索。搜索使用全文（token 级）匹配：空格分隔的关键词之间为 AND，每个关键词匹配完整 token 或其前缀；搜索结果中的每个分组都至少包含一条匹配记录。搜索进行中时搜索框下方会显示一条动画进度条，完成后自动隐藏。
- 大日志目录分页加载。
- 对相关 Agent 工作流进行任务分组。
