...
describe("connectLogDatabase", () => {
  it("creates the complete current schema", async () => {
    ...
    expect(database.pragma("table_info(records)") as { name: string }[]).not.toContainEqual(
      expect.objectContaining({ name: "first_byte_ms" }),
    );
...
describe("database backup", () => {
  it("checkpoints WAL data and creates a readable SQLite backup", async () => {
    ...
    expect(readSchemaVersion(backup)).toBe(9);
    backup.close();
  });
