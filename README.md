...
| 成本统计 | 按任务显示总费用、已计价/未计价请求、价格规则和 token 明细；每条请求显示其费用占比。缺少可靠用量或价格时标记为未计价，不会当作免费。点击任务头部控制列的 ⓘ 图标按钮打开“任务明细”面板查看费用，面板内另附“Token 趋势”折线图，按请求序号展示每个请求的 token 总量（请求 + 响应）。 |
