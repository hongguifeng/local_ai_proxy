...
- [ ] 列出当前公开 CLI 参数、默认路径和环境变量。
- [ ] 列出配置字段、默认值和跨字段约束。
- [ ] 列出管理 API request/response 示例。
- [ ] 列出代理 method、path、header、路由和 body 变换规则。
- [ ] 列出 traffic record 状态和 SQLite 字段语义。
- [ ] 列出任务匹配策略和支持的 endpoint。
- [ ] 标记每项为“必须保留”“允许改变”或“明确删除”。
- [ ] 把已知原型缺陷列为禁止复制的行为。
...
- [ ] 实现 request 顶层 model 提取。
- [ ] 实现 enabled target 的有序匹配和 default fallback。
- [ ] 实现 model 改写。
- [ ] 实现顶层字段 strip/inject。
- [ ] 明确 inject 与 model rewrite 的执行顺序。
- [ ] 无效 JSON 和非 object JSON 原样处理。
- [ ] 返回结构化 transform metadata 供日志使用。
...
- [ ] 选择支持 streaming 的 ZIP 库并记录理由。
- [ ] 从 repository 分页读取，不加载全部 task/body。
- [ ] 生成稳定、安全的相对文件名，防止 Zip Slip。
- [ ] 支持客户端取消时停止查询和压缩。
- [ ] 限制并发导出任务。
- [ ] 添加大数据集内存测试和 ZIP 内容测试。
