...
创建日期：2026-09-11。当前进度：14 / 14 项完成。
...
| [x] | PRICE-14 | 使用文档与最终交付检查 | PRICE-13 | 完成 |
...
状态：完成
完成日期：2026-09-11
实现内容与实际文件：在 `README.md` 与 `README.cn.md` 补充模型 token 定价和历史费用的配置示例、匹配规则、请求时价格冻结、四类 token 桶、历史 task/request 明细以及旧记录 `legacy_record` 行为说明；新增 README 回归，确保示例可由实际 schema 解析且两种 README 继续引用最终 UI 截图。
测试映射：P14-01、P14-02 → `test-node/docs/readme.test.ts` 的 `documents a schema-valid model pricing example and final screenshots`，覆盖中英文配置示例、冻结/历史说明及截图引用；P14-03 → 完整项目 `npm run check`，覆盖 API、持久化、导出、UI、端到端和截图回归。
验证命令与结果：`npm run check` 通过；格式检查、lint、typecheck、82 个测试文件共 606 个测试、生产构建均通过，JUNIT 报告位于 `test-results/vitest-junit.xml`。
验收结论：通过；14 项功能、文档和自动化测试均已完成，最终行为与设计文档一致。
文档更新：`README.md`、`README.cn.md`、TODO 执行记录；性能和端到端结果见 `docs/model-token-pricing-test-report.md`。
UI 基线：此前 PRICE-09～12 已在各自 UI 提交前执行 `npm run regen:ui-baselines`；本项仅更新文档，最终截图与基线哈希无变化。
设计偏差与迁移影响：无；价格上线前记录保持 `legacy_record`，不按当前价格回算。
提交定位：`feat(pricing): [PRICE-14] document pricing history`。
补充修复提交：无。
遗留问题：无。
...
- [x] 14 项功能与测试全部验收，每项都有代码/测试/文档提交记录。
- [x] 完整项目检查通过，端到端与性能报告可复现。
- [x] 中英文 README 截图及 UI 基线哈希与最终代码一致。
- [x] 设计文档、使用说明、API 示例与实际实现一致。
- [x] 提交后核对工作区，说明未提交内容，不把其他改动算作本功能交付。
