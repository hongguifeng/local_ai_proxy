...
- [ ] `BASE-001` P0 记录当前 commit、Python 版本和 66 项测试结果。
  - 验收：仓库内有可复现命令和测试报告。
...
- [ ] `BASE-010` P0 测试 GET/POST/PUT/PATCH/DELETE/OPTIONS/HEAD。
...
- [ ] `BOOT-001` P0 创建 `package.json`，声明 Node 24 engine。
...
- [ ] `BOOT-006` P0 添加 `.node-version` 或 `.nvmrc`。
...
- [ ] `CI-001` P0 在 GitHub Actions 增加 Node 24 job。
