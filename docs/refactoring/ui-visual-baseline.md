...
- 表下为“Whole-task cost”提示文案，随后是三张 SVG 折线图（x 均为请求序号，一个点一个请求，点带悬停 title），每图下方各有一行说明：Token 总量趋势（y=每请求 token 总量，无 token 数的请求不显示）、Token 输出趋势（y=自任务开始到每个请求为止的累计输出 token）、费用趋势（y=累计费用）。三图按指标语义着色：Token 总量为青色（#0e7490，与“总 Token”卡片同色）、Token 输出为蓝色（#315a82，与请求数量蓝色同系）、费用保持绿色（绿色专属于费用语义）；各节标题左侧有与折线同色的小圆点，帮助快速区分三个分区。
