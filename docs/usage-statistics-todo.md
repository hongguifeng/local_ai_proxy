...
| --- | --- | --- | --- | --- |
统计数据模型、索引与迁移 | 无 | 未开始 |
repository 总量与分组查询 || 未开始 |
repository 趋势 bucket 查询 || 未开始 |
UsageStatisticsService 合并与口径 | STAT-02、03 | 未开始 |
管理 API、schema 与鉴权 || 未开始 |
CSV 导出接口 || 未开始 |
页面导航、筛选和状态管理 || 未开始 |
总体分布饼图与指标切换 || 未开始 |
| [x] | STAT-09 | 明细表格与趋势柱状图 || 未开始 |
