...
创建日期：2026-09-13。当前进度：8 / 12 项完成（STAT-07 仍需浏览器级交互测试）。
...
| [x] | STAT-07 | 页面导航、筛选和状态管理 | STAT-05、06 | 已完成 |
| [x] | STAT-08 | 总体分布饼图与指标切换 | STAT-07 | 已完成 |
| [x] | STAT-09 | 明细表格与趋势柱状图 | STAT-07、08 | 已完成 |
| [ ] | STAT-10 | 中英文 UI | STAT-08、09 | 部分完成：待统计页英文截图及残留检查 |
...
完成记录：统计页图表 smoke、粒度选择、地址/模型筛选参数化测试已通过（`test-node/ui/admin-ui.test.ts`）。
...
完成记录：SVG 饼图、Top 9 + 其他、无数据提示、指标切换、图例交互及静态回归测试已完成。

修复记录：为 SVG 饼图切片设置 `fill="none"` 和环形描边，避免默认黑色填充覆盖整个图表。

界面修复记录：限制饼图 SVG 为 180×180，移除错误的整块渐变背景，并将趋势柱分段设为可见的堆叠块，修复图表撑满页面和布局异常问题。
...
完成记录：粒度选择、四类 token 分段柱、费用柱、悬停详情和一致性测试已完成；测试提交 `f73edb5`。
...
- **STAT-05**：已增加 Fastify 枚举、时间格式和严格参数 schema，并增加 schema 契约回归测试；仍需 overview/trend/options 的 200/400/401/404 集成测试。
- **STAT-06**：增加 CSV 内容一致性、特殊字符、中文、空结果和公式注入测试。
- **STAT-07**：已接入 options API、地址/模型筛选联动、加载状态、空结果提示、API 错误提示及重试按钮，并通过统计页图表、粒度、地址和模型筛选浏览器级 smoke 测试；仍需完整筛选联动交互测试。
- **STAT-08**：已替换为 SVG 饼图并支持 top 9 +“其他”、无数据提示及图例隐藏/恢复交互；已增加图例控件静态回归测试。
- **STAT-09**：已加入日/周/月粒度选择、四类 token 趋势分段柱、费用指标柱和完整悬停详情，并增加表格/图表 token 合计一致性测试。
- **STAT-10**：已补充统计分布标题、空态文案、总体指标标签、筛选器选项及趋势表头的中英文切换；统计页英文 smoke 截图已生成并人工检查，仍需全页面残留扫描。
- **STAT-11**：统计与静态回归测试集共 10 个测试已通过，含 10,000 条记录基准和 366 点上限；仍需跨日志目录、无正文读取和真实页面流程的端到端/性能回归测试。
- **STAT-12**：已补充中英文 README 基础说明及中英文 API 参数文档；portable 构建链路已验证并生成安装包，仍需最终验收记录。
