...
| `model_prices` | array | 空 | 模型价格规则；每项包含 `model_pattern`、可选的 `price_multiplier`（省略时为 `1`）与四个元 / 100 万 token 的十进制字符串单价：`input_per_million`、`output_per_million`、`cache_read_per_million`、`cache_write_per_million`。配置单价和倍率必须为 0 至 1,000,000、最多 6 位小数；实际计费与历史展示使用配置单价乘以倍率后的价格；同一 target 内 pattern 不可重复。 |
...
- 通过开关启停代理，绿色圆点显示正在运行。
- 添加/删除上游目标，目标至少保留一个。
- 拖动手柄调整上游目标顺序（焦点在手柄上时可用方向键移动）；顺序决定模型映射的匹配优先级。
- 选择默认目标。
...
- 全选/取消全选、清理选中、清理单请求、导出、手动刷新按钮；自动刷新开关紧跟在刷新按钮之后，与其他按钮一样固定在同一位置（文字位于复选框下方，纵向排列，复选框与文字居中对齐），按钮行不换行，宽度不足时自动刷新从右侧被裁剪，不与前面的按钮重叠。
- task 可独立勾选用于清理；“全选”按钮只选中当前页加载出来的 task（每页 100 个），全部选中后按钮切换为“取消全选”，再次点击取消全部选中。
- “清理单请求”按钮按当前搜索词分页遍历 /api/logs（每页 100 个，含未加载的页），找出二级列表只含一个请求（request_count=1）的 task 并整体清理；遍历与清理期间按钮禁用并显示“清理中…”，搜索框下方显示不确定进度条，完成后恢复按钮并隐藏进度条；无匹配时提示“没有仅含一个请求的任务”，清理后刷新列表并提示已清理的 task 数量。
