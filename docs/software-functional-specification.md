...
| F-22 | 自动刷新 | History 页面默认每 3 秒刷新，页面不可见时跳过实际加载 |
...
| `timeout` | number | `600` | 上游连接和读取超时秒数 |
...
4. 在目标的 `model_mappings` 中执行字符串精确匹配，区分大小写，不支持通配符或正则。
5. 第一个匹配项立即选定目标。
...
- 非默认目标可启停。
- “更多配置”展开 timeout、log root、脱敏、Header、删除字段、注入字段。
- 保存配置后显示 Toast。
- 目标列表重绘时保留横向滚动位置。
...
- 支持复制完整 JSON。
- 支持显示/隐藏请求和响应元信息。
- 自动刷新 selected record 时保留树节点展开状态和视图选项。
