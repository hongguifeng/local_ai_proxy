...
### 2026-09-08：搜索结果批量附带二级列表预览

- 带关键词的 `/api/logs` 响应为当前页每个分组附带 `preview`，结构与组内记录分页
  接口一致，包含最新 20 条匹配记录、匹配总数、`next_offset` 和 `has_more`。
  不带关键词时仍按需加载组内记录。
- 预览按数据库批量查询：一次非相关 FTS 查询匹配当前页所有分组的记录，使用窗口函数
  计算各组数量和顺序，仅读取前 20 条的列表字段，不读取请求或响应正文。多日志目录先
  全局排序分页，再为可见分组获取预览，避免逐组重复全文搜索。
- UI 将预览直接作为当前搜索词的记录缓存。搜索完成后展开分组无需再发请求；超过
  20 条时通过现有“加载更多”接口从 `next_offset` 继续。搜索词或分组摘要变化时使用
  新预览，普通自动刷新保留已加载的更多记录。
- 取舍：首次搜索增加一次批量摘要查询和每组最多 20 条摘要的响应体，消除搜索完成后
  逐组展开的额外全文查询与网络等待。记录详情仍在选中记录后加载。
- 回归覆盖：一万条记录下的批量预览性能、AND 匹配和分页连续性、多目录预览，以及
  浏览器中展开不发二次请求、切换关键词替换预览、从预览偏移继续加载。
