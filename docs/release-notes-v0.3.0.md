...
这是应用从 Python 完整切换到 Node.js 24 + TypeScript + Electron 的首个正式版本。代理、管理 UI、历史记录、SQLite 数据、CLI 和 Windows Tray 功能均已迁移；运行和开发不再需要 Python。本版本基于已通过完整远程与手工验收的 `v0.3.0-rc.5`。
...
- Windows x64 用户可选择 NSIS installer 或 portable EXE；两者均内置运行时，不要求预装 Node.js。
- 轻量 CLI ZIP 需要 Node.js 24；解压后执行 `npm ci --omit=dev`，再使用 `npm start -- --port 12344` 启动。
- 源码开发需要 Node.js 24；执行 `npm ci` 后可运行、测试和打包。
- SQLite 使用 `better-sqlite3`，Windows 产物已针对 Electron ABI 重建并完成 FTS5 smoke。

## 迁移与回滚

正式切换前请按 [迁移回滚说明](migration-rollback.md) 备份配置、`traffic.db`、WAL/SHM，并确认至少有数据库大小 2.4 倍的可用空间。现有 `proxies.json` 和数据库可由 Node 版本直接读取；7.58 GB 活跃真实数据库已完成 online backup、迁移、关系校验、SHA-256 回滚和重新打开验证。

## 验证摘要

- 72 个 Node test files、497 个测试在 Linux 和 Windows 全部通过（Linux 上 1 个 Windows 专用断言按设计跳过）。
- Windows x64 installer 和 portable 构建成功；portable 健康检查、优雅关闭和端口释放通过。
- GitHub Release 提供 installer、portable EXE、CLI ZIP 和 `SHA256SUMS.txt`；产物名与 checksum 条目一致。
