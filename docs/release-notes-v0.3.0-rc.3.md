...
相较此前候选版本，本版本修复了 Windows CI 的路径可移植性、Chrome 启动路径、浏览器语言状态隔离和 Git checkout 行尾一致性问题。随后发现带空格的远程附件名会被 GitHub 规范化，导致 checksum 文件名不匹配，因此本版本已由 `v0.3.0-rc.4` 替代，不作为有效候选版本。
